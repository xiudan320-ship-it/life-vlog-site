...
function closestCard(target) {
  return target instanceof Element ? target.closest(".shopping-card") : null;
}

function isInteractiveTarget(target) {
  return target instanceof Element && Boolean(
    target.closest("button, a, input, textarea, select, [data-shopping-menu]")
  );
}

export function createShoppingInteractions({
  listElement,
  onAdd,
  onOpenImage,
  onOpenDetail,
  ...
  function closeSwipe(card = activeSwipeCard) {
    ...
    card.style.removeProperty("--shopping-swipe-x");
  ...
  function openSwipe(card) {
    ...
    card.style.setProperty("--shopping-swipe-x", `-${SWIPE_OPEN_DISTANCE}px`);
  }
  ...
  function moveDraggedCard(clientY) {
    ...
    const cards = [...listElement.querySelectorAll(".shopping-card:not(.is-dragging)")];
  ...
  function finishReorder() {
    if (!pointerState?.dragging) return;
    const ids = [...listElement.querySelectorAll(".shopping-card[data-shopping-id]")]
      .map((card) => card.dataset.shoppingId)
      .filter(Boolean);
  ...
  function handleClick(event) {
    ...
    const addButton = target instanceof Element ? target.closest("[data-add-shopping]") : null;
    if (addButton) {
    ...
    if (!card) return;
    const id = card.dataset.shoppingId;
    const imageButton = target instanceof Element ? target.closest("[data-shopping-image]") : null;
    if (imageButton) {
      event.preventDefault();
      onOpenImage?.(imageButton.dataset.shoppingImage, imageButton.dataset.shoppingImageAlt || "商品图片");
      return;
    }
    const toggleButton = target instanceof Element ? target.closest("[data-toggle-shopping]") : null;
    if (toggleButton) {
      event.preventDefault();
      closeSwipe(card);
      onToggle?.(toggleButton.dataset.toggleShopping || id);
      return;
    }
    const menuButton = target instanceof Element ? target.closest("[data-shopping-menu]") : null;
    if (menuButton) {
      event.preventDefault();
      onOpenMenu?.(menuButton.dataset.shoppingMenu || id);
      return;
    }
    const swipeButton = target instanceof Element ? target.closest("[data-shopping-swipe-action]") : null;
    if (swipeButton) {
      event.preventDefault();
      closeSwipe(card);
      if (swipeButton.dataset.shoppingSwipeAction === "toggle") onSwipeToggle?.(id);
      if (swipeButton.dataset.shoppingSwipeAction === "delete") onRemove?.(id);
      return;
    }
    if (isInteractiveTarget(target)) return;
    if (card.dataset.shoppingDetail !== "false") onOpenDetail?.(id);
  }

  function handlePointerDown(event) {
    if (event.pointerType === "mouse" && event.button !== 0) return;
    const card = closestCard(event.target);
    if (!card || isInteractiveTarget(event.target) || !canReorder(card) && event.pointerType !== "touch") return;
    cancelLongPress();
    pointerState = {
      card,
      ...
      swiping: false,
      dragging: false,
      longPressTimer: canReorder(card)
        ? window.setTimeout(beginReorder, LONG_PRESS_DELAY)
        : null,
    };
  }
  ...
  function handlePointerMove(event) {
    ...
    if (dx >= 0) {
      ...
      pointerState.card.style.removeProperty("--shopping-swipe-x");
      return;
    }
    pointerState.swiping = true;
    pointerState.card.classList.add("is-swiping");
    pointerState.card.style.setProperty("--shopping-swipe-x", `${Math.max(dx, -SWIPE_OPEN_DISTANCE)}px`);
    event.preventDefault();
  }
