...
assert.match(weekendBoardCss, /\.weekend-completion-drop > \.upload-native-trigger[\s\S]*?color: var\(--lv-on-accent\)/);
assert.match(weekendBoardCss, /\.weekend-completion-dialog footer[\s\S]*?grid-template-columns: minmax\(124px, \.72fr\) minmax\(0, 1\.28fr\)/);
assert.match(weekendBoardCss, /#weekendCompletionSubmit[\s\S]*?border-color: color-mix\(in srgb, var\(--lv-accent\)/);
