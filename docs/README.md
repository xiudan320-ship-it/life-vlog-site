...
1. 先缩小候选范围：通常优先使用 `rg -l`，并尽量限定目录、文件类型和关键词；已知具体文件或符号时可直接定位。
2. 再找符号：需要进一步定位时，在候选文件中使用 `rg -n` 查找函数、类、文案或选择器。
3. 最后局部读取：只读取命中的相关函数、类或附近代码。
4. 命中过多时继续缩小目录、文件类型或关键词范围，不批量展开全部结果。
