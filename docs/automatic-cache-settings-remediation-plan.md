...
> 状态：待实施。本文记录 2026-09-02 对“设置 → 存储与数据”页面的静态代码审计结果、修复边界与验收标准，不代表功能已经修复或上线。
...
- [ ] 页面首次打开不出现 `undefined MB`、`NaN`、空状态或短暂错误覆盖最终状态。
- [ ] 自动缓存整行可用鼠标、触摸、Enter 和 Space 操作。
- [ ] 一次操作只切换一次，行内状态、toast、`aria-pressed` 与持久化值完全一致。
- [ ] 关闭设置、重新打开和页面刷新后状态正确恢复。
- [ ] 容量弹窗保存后摘要立即更新，且两个容量都为有限整数。
- [ ] 手机、横屏、平板、桌面均无截断、横向溢出或点击区域缩小。
- [ ] 重复初始化不重复监听；无控制台错误和未处理异常。
- [ ] `layout-settings-controller.js` 不再拥有缓存策略/容量的业务读取。
- [ ] 无兼容层、旧字段别名、双写或显示层 fallback。
- [ ] 相关测试、文档、变更日志和构建门禁全部通过后才允许部署。
