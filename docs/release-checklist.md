...
`test:browser`/`c-performance-regression.mjs` 另外验证普通视频不触发有声 autoplay、失败可重试、gallery 同步读取秘藏表而不加载秘藏 route、30 轮快速路由 latest-wins，以及既有周末/购物/回复交互。所有 signed-in 场景均使用 `tests/fixtures/cloudflare-api-fixture.mjs` 的内存 fixture，不使用真实账户或真实业务数据。
