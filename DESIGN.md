...
- 页面改版、新页面或视觉方案比较优先使用 Figma；局部修复直接改代码即可。具体选择遵循 [AGENTS.md](AGENTS.md) 第 9 条，进入 Figma 任务后再读 [Figma 工作流](docs/FIGMA_WORKFLOW.md) 对应阶段。
- `ui-ux-pro-max` 是按需咨询的设计参考，不要求每个 UI 任务读取、搜索或生成设计系统。
...
- 局部修改只检查受影响的代表宽度；只有共享布局、断点或全局样式变化才运行 375、390、430px，平板 768px，桌面 1024、1440px 的完整矩阵。影响超宽侧栏时补充超宽检查。只改文档时检查链接、规范一致性和 diff，无需重跑整站视觉验收。
