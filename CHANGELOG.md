...
- 日记画廊阶段二样式改为异步 chunk，入口 CSS 保持在 256 KiB 内；入口 JS gzip 预算明确调整为 123 KiB 以容纳默认画廊的状态与上传逻辑。
- 设置中心缓存操作统一由离线设置 controller 幂等接线，全部清理保留账号、个人设置、草稿、上传队列和容量偏好；手机标题/搜索、顶部分页与工具排序调整为更紧凑且可触控的布局。
- 设置搜索按管理员能力过滤 R2 统计；Service Worker 检查增加超时恢复，PWA 安装事件支持先于设置 DOM 到达，子窗口返回恢复触发项焦点和滚动位置。
- 设置可靠性回归补齐合成账号下的外观持久化、账户/家庭动作、备份/回收站、诊断和上传队列实际结果检查；真实账户、邮件、通知、设备安装和云端 destructive path 仍明确隔离。
...
- 修复异步加载和异步重绘中的桌面日记“更多”菜单丢失方向键焦点移动的问题，并在重绘后恢复打开状态。
- 修复缓存统计在 CacheStorage 读取失败时把异常吞掉并伪装成 0 KB 成功的问题；设置页现在显示读取失败，CacheStorage 恢复后可由原刷新按钮重试，真正可读的空缓存仍报告为 0。

### Release

- 已从 `main` 提交 `73b89b663caa9bb744af59cdde1f42874b478013` 发布缓存统计错误传播与设置可靠性回归补全；固定 preview 为 `https://codex-preview.life-vlog-site.pages.dev`，正式入口为 `https://life-vlog-site.pages.dev`，production deployment 为 `1c299a81.life-vlog-site.pages.dev`（`https://1c299a81.life-vlog-site.pages.dev`）。Worker `life-vlog-r2-upload` 版本为 `c902a792-e38f-4f83-9bc1-c881b2fb2144`；入口为 `index-DSPFE7JW.js`（SHA-256 `2d8c974099ee209fc5e46ade3754f8e5bd948c9796cbbbd097cfe545b0debb520`），样式为 `index-4xRD1Tj0.css`（SHA-256 `afe5762fd9793c4d3a9212ad0ef2ebdab9b3bef4676149db98e4defae7d5257a`），`sw.js` SHA-256 为 `522683ecb72795ead58c2406c6c8c47ed5d95537922203737efdac5158055359`，Workbox 预缓存条目为 34；`pnpm install --frozen-lockfile`、完整本地回归、preview/production 的 Worker CORS、Axe critical/serious 和确定性 release smoke 均通过。
- 已从 `main` 提交 `67597a175b334bcf7bb0f38dd5afb3c53b867e2d` 发布到 Cloudflare Pages；固定 preview 为 `https://codex-preview.life-vlog-site.pages.dev`，本次 preview deployment 为 `3d7b32dd-1237-45c5-8c26-d12259ada91f`（`https://3d7b32dd.life-vlog-site.pages.dev`）；正式入口为 `https://life-vlog-site.pages.dev`，production deployment 为 `8ce57823-c826-474b-b4d2-559fe2aaa887`（`https://8ce57823.life-vlog-site.pages.dev`）。Worker `life-vlog-r2-upload` 版本为 `e5cebf84-cf84-4efb-bae5-16aecf528269`；入口为 `index-B38tWKFm.js`（SHA-256 `989a1d310484ea4992a27eee097eaed8ed1af879e856290d7403e1fdd50fc065`），样式为 `index-4xRD1Tj0.css`（SHA-256 `afe5762fd9793c4d3a9212ad0ef2ebdab9b3bef4676149db98e4defae7d5257a`），`sw.js` SHA-256 为 `c5d6ca8a36eb861dea8f78d003474b53c3363e0737353025fa3e0799082def66`，Workbox 预缓存条目为 34；`pnpm install --frozen-lockfile`、完整本地回归、preview/production 的 Worker CORS、Axe critical/serious 和确定性 release smoke 均通过。
...
- 已从 `main` 提交 `70ba6b526f5ef36c778bc332a359b1bff3064393` 发布缩略图可读性修复；preview deployment 为 `25dd4b30-bc51-4c86-a1a5-bb61908ecfd8`（固定 alias `https://codex-preview.life-vlog-site.pages.dev`），正式入口为 `https://life-vlog-site.pages.dev`，production deployment 为 `39ec6372-5954-4834-a850-ea928ec9301c`（`https://39ec6372.life-vlog-site.pages.dev`）。Worker `life-vlog-r2-upload` 版本为 `f489e3a9-f4f4-472a-96a5-d579cd1b945e`；入口为 `index-1zOMqktA.js`（SHA-256 `bebc563711128cfe9d165296f50e5c7d0e8fdac917ceae0c83d6b725a508bbba`），样式为 `index-4xRD1Tj0.css`（SHA-256 `afe5762fd9793c4d3a9212ad0ef2ebdab9b3bef4676149db98e4defae7d5257a`），`sw.js` SHA-256 为 `4a5fe53635c7e06d2c8a70b6bcbc377dfcbd0300d0fbc9a181a69dc912e91d83`，Workbox 预缓存条目为 34；preview/production 的 Worker CORS、Axe critical/serious 和确定性 release smoke 均通过。

### Documentation

- 新增并执行设置功能恢复与视觉整理规划：记录缓存按钮缺少绑定、安装入口懒加载时序、诊断复制误报和子窗口焦点问题，并同步逐项验收结果、Figma 只读对照和移动端密度/触控边界。
- 更新第二阶段 UI 与操作体验规划的实施记录、Figma 节点和真实验收边界；B/D 的 Figma 写入受 Starter MCP 调用额度限制，代码实现不受影响。
- 新增并执行首页 UI 优化规划：存储统计迁入设置、紧凑心情概览、桌面日记操作菜单及手机工具/搜索调整均已完成 Figma、代码和确定性验收。
- UI 工作流改为页面方案优先使用 Figma、局部修复直接改代码；`ui-ux-pro-max` 降为按需参考。新增 Figma 设计、现有代码实现、浏览器验收和首页试点规划，并接入按需文档路由；本次仅修改文档，不触发网站部署。
- 按 GPT-6 Astra 技能审计收紧 Figma、Canva、DOCX 与 OpenAI Docs 的触发和执行边界：单组件与纯设计稿走最小路径，素材可直接复用，相关写入串行，验收按影响范围执行；明确授权的 Canva 修改不重复请求保存确认。

### Changed

- 将管理员 R2 存储统计从首页账户同步和顶部导航迁入设置页“存储与数据”，在设置 DOM 挂载并进入该分区后才懒加载和请求；保留管理员权限、失败重试、账号切换隔离与迟到响应保护。
- 压缩普通桌面今日心情与月历概览，成员卡改为自然高度、月历保持完整 4～6 周缩略网格；移动端、短横屏和 1900px 以上右侧栏规则保持独立。
- 桌面日记卡片保留收藏，将编辑、精选、置顶、分类和删除集中到键盘可操作的“更多”菜单；菜单控制器按画廊挂载后动态加载，避免扩大首页主入口。
- 手机工具栏首屏前三项和搜索区域调整为完整可读、紧凑可触控布局，并保留排序、滑动、筛选与大字模式行为。
