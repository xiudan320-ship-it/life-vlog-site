...
- 将心情瓶升级为 `360×440` 纵向暖色玻璃构图；表情按数量进入慢速分批计划，整只瓶子改为可点击/键盘重播的原生按钮，并在重播中断、月份切换和离开路由时清理动画。
...
- 增加移动端留言线程与心情瓶物理下落 V3 规划：取消深层回复累计横移，规定全宽扁平行模型、留言表单布局，以及具有瓶壁/瓶底/表情间碰撞、确定性重播和慢速自然节奏的心情瓶模拟。
